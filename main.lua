...
local Cont = Instance.new("ScrollingFrame", M); Cont.Size = UDim2.new(1, -110, 1, -10); Cont.Position = UDim2.new(0, 105, 0, 5); Cont.BackgroundTransparency = 1; Cont.CanvasSize = UDim2.new(0, 0, 11, 0); Cont.ScrollBarThickness = 4
...
-- [[ 2. AUTO FARM (SOLIX 24H GUARANTEE) ]] --
AddB("--- AUTO FARM ---", Color3.new(0, 0.3, 0.5), function() end)

AddB("Solix Hub (24H KEY ACCESS) 🔥", Color3.fromRGB(120, 0, 255), function() 
    -- Ссылка копируется мгновенно
    setclipboard("https://ads.luarmor.net/get_key?for=Solixhub_Free_KeySystem-OWlLHDMCHADk")
    
    -- Уведомление на экран (висит 10 секунд)
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "CANDY-BOMB SYNC",
        Text = "ССЫЛКА СКОПИРОВАНА! Вставь её в Google/Chrome.",
        Duration = 10
    })
...
_G.TP_LOOP = false
AddB("STOP TP", Color3.fromRGB(150, 0, 0), function() _G.TP_LOOP = false end)
for _, v in pairs(game.Players:GetPlayers()) do
    if v ~= p then 
...
AddB("AIMBOT", Color3.fromRGB(50, 50, 50), function()
