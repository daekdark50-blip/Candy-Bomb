-- [[ CANDY-BOMB: ALL-IN-ONE FINAL EDITION ]] --
...
-- 1. ТВОЯ СОБСТВЕННАЯ КРОВАВАЯ ЗАГРУЗКА (4 СЕКУНДЫ)
local function StartLoading()
    ...
    tw.Completed:Connect(function() 
        task.wait(0.2); LG:Destroy(); CreateCandyHub() 
    end)
end
...
function CreateCandyHub()
    ...
    local function AddTab(txt, target)
        local b = Instance.new("TextButton", Side); b.Size = UDim2.new(1, -10, 0, 45); b.Text = txt; b.BackgroundColor3 = Color3.fromRGB(25, 25, 25); b.TextColor3 = Color3.new(1,1,1); b.Font = Enum.Font.SourceSansBold; b.TextSize = 12; Instance.new("UICorner", b)
    ...
    local function AddB(pnt, txt, clr, cb)
        local b = Instance.new("TextButton", pnt); b.Size = UDim2.new(1, -10, 0, 40); b.Text = txt; b.BackgroundColor3 = clr; b.TextColor3 = Color3.new(1, 1, 1); b.Font = Enum.Font.SourceSansBold; b.TextSize = 14; Instance.new("UICorner", b); b.MouseButton1Click:Connect(cb); return b
    end

    -- ВКЛАДКА 1: 11 СКРИПТОВ
    local function s(n, u) AddB(P1, n, Color3.fromRGB(40,40,40), function() loadstring(game:HttpGet(u))() end) end
    ...
    -- ВКЛАДКА 2: MY HUB (20 ФУНКЦИЙ С ON/OFF)
    for i=1, 20 do
        local state = false
        local b; b = AddB(P2, "FUNCTION "..i..": OFF", Color3.fromRGB(100,0,0), function()
            state = not state; b.Text = "FUNCTION "..i..": "..(state and "ON" or "OFF")
            b.BackgroundColor3 = (state and Color3.fromRGB(0,120,0) or Color3.fromRGB(100,0,0))
        end)
    end

    -- ВКЛАДКА 3: AUTO FARM (ПО ТВОЕМУ СПИСКУ)
    AddB(P3, "ITEM COLLECTOR", Color3.fromRGB(0, 80, 200), function() 
    ...
    AddB(P3, "GOD MODE", Color3.fromRGB(0, 80, 200), function() p.Character.Humanoid:SetStateEnabled(Enum.HumanoidStateType.Dead, false) end)
    AddB(P3, "--------------------------", Color3.fromRGB(35, 35, 35), function() end)
    
    -- Твои главные скрипты в самом низу
