-- [[ CANDY-BOMB: BLOODY PERFECT EDITION ]] --
...
local function StartLoading()
    ...
    tw:Play()
    tw.Completed:Connect(function() task.wait(0.3); LG:Destroy(); CreateCandyHub() end)
end
...
function CreateCandyHub()
    ...
    -- 📁 Вкладка 2: 20 ФУНКЦИЙ MY HUB
    AddB(P2, "-- MOVEMENT --", Color3.fromRGB(150,0,0))
    local ij_on = false; local ijb = AddB(P2, "INF JUMP: OFF", Color3.fromRGB(100,0,0), function() 
        ij_on = not ij_on; ijb.Text = "INF JUMP: "..(ij_on and "ON" or "OFF"); ijb.BackgroundColor3 = (ij_on and Color3.fromRGB(0,120,0) or Color3.fromRGB(100,0,0))
    end); uis.JumpRequest:Connect(function() if ij_on then p.Character.Humanoid:ChangeState("Jumping") end end)

    local nc_on = false; local ncb = AddB(P2, "NOCLIP: OFF", Color3.fromRGB(100,0,0), function() 
        nc_on = not nc_on; ncb.Text = "NOCLIP: "..(nc_on and "ON" or "OFF"); ncb.BackgroundColor3 = (nc_on and Color3.fromRGB(0,120,0) or Color3.fromRGB(100,0,0))
    ...
    AddB(P2, "ANTI-AFK", Color3.fromRGB(40,40,40), function() p.Idled:Connect(function() game:GetService("VirtualUser"):CaptureController(); game:GetService("VirtualUser"):ClickButton2(Vector2.new()) end) end)
    AddB(P2, "RESPAWN", Color3.fromRGB(40,40,40), function() p.Character:BreakJoints() end)
    
    AddB(P2, "-- VISUAL --", Color3.fromRGB(150,0,0))
    local fb_on = false; local fbb = AddB(P2, "FULL BRIGHT: OFF", Color3.fromRGB(100,0,0), function() 
        fb_on = not fb_on; fbb.Text = "FULL BRIGHT: "..(fb_on and "ON" or "OFF"); fbb.BackgroundColor3 = (fb_on and Color3.fromRGB(0,120,0) or Color3.fromRGB(100,0,0))
        game.Lighting.Brightness = (fb_on and 2 or 1)
    end)
    AddB(P2, "STOP WATCHING", Color3.fromRGB(120,0,0), function() workspace.CurrentCamera.CameraSubject = p.Character.Humanoid end)

    for i=1, 8 do AddB(P2, "WATCH PLAYER SLOT " .. i, Color3.fromRGB(30,30,30), function() end) end
    ...
    AddB(P3, "2. AUTO FARM (TSUNAMI)", Color3.fromRGB(0,80,200), function() loadstring(game:HttpGet("https://raw.githubusercontent.com/osakaTP2/OsakaTP2/refs/heads/main/Escape%20Tsunami%20For%20BrainrotsDelta"))() end)
    ...
    AddB(P3, "4. ITEM COLLECTOR", Color3.fromRGB(0,80,200), function() for _,v in pairs(workspace:GetDescendants()) do if v:IsA("TouchTransmitter") then firetouchinterest(p.Character.HumanoidRootPart, v.Parent, 0) end end end)
    AddB(P3, "5. SPEED AUTO", Color3.fromRGB(0,80,200), function() p.Character.Humanoid.WalkSpeed = 100 end)
end
